...
export async function searchMovieTorrents({ indexer, name, year, imdbId, supportedParams = [] }) {
  ...
  if (!items) {
    ...
    if (!items.length && imdbPart && !supports('imdbid')) {
      ...
      items =
        (await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, retryQuery))?.rss?.channel?.item || [];
    }
    if (items.length) {
...
export async function searchSerieTorrents({ indexer, name, year, imdbId, supportedParams = [] }) {
  ...
  if (!items) {
    ...
    if (indexer === 'toloka') {
      ...
      const res = await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, query);
      items = res?.rss?.channel?.item || [];
      ...
      if (!items.length && baseName !== name) {
        ...
        items =
          (await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, altQuery))?.rss?.channel?.item || [];
      }
    }
    if (items.length) {
      cache.set(cacheKey, items, { ttl: 3600 * 36 });
...
export async function searchSeasonTorrents({ indexer, name, year, season, imdbId, supportedParams = [] }) {
  ...
  if (!items) {
    if (indexer === 'toloka') {
      const queries = [`${name} S${numberPad(season)}`, `${baseName} S${numberPad(season)}`, baseName].filter(Boolean);
      for (const q of queries) {
        const res = await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, { t: 'search', q });
        items = res?.rss?.channel?.item || [];
        if (items.length) break;
      ...
      const query = {
        t: 'tvsearch',
        q: imdbPart ? `${name} ${imdbPart} S${numberPad(season)}` : `${name} S${numberPad(season)}`,
        cat: CATEGORY.SERIES,
      };
      if (imdbId && supports('imdbid')) query.imdbid = imdbId.replace(/^tt/, '');
...
export async function searchEpisodeTorrents({ indexer, name, year, season, episode, imdbId, supportedParams = [] }) {
  ...
  if (!items) {
    if (indexer === 'toloka') {
      const queries = [baseName].filter(Boolean);
      for (const q of queries) {
        const res = await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, { t: 'search', q });
        items = res?.rss?.channel?.item || [];
        if (items.length) break;
      ...
      const query = {
        t: 'tvsearch',
        q: imdbPart
          ? `${name} ${imdbPart} S${numberPad(season)}E${numberPad(episode)}`
      ...
      if (!items.length && baseName !== name) {
        const altQuery = {
          t: 'tvsearch',
          q: `${baseName} S${numberPad(season)}E${numberPad(episode)}`,
          cat: CATEGORY.SERIES,
        };
        items =
          (await jackettApi(`/api/v2.0/indexers/${indexer}/results/torznab/api`, altQuery))?.rss?.channel?.item || [];
...
export async function getIndexers() {
  const res = await jackettApi('/api/v2.0/indexers/all/results/torznab/api', { t: 'indexers', configured: 'true' });
