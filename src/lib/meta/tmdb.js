...
export default class Tmdb {
  ...
  async getMovieById(id, language) {
    const searchId = await this.#request(
      'GET',
      `/3/find/${id}`,
      { query: { external_source: 'imdb_id', language: language || 'en-US' } },
      { key: `searchId:${id}:${language || '-'}`, ttl: 3600 * 3 }
    );
  ...
  async getEpisodeById(id, season, episode, language) {
    const searchId = await this.#request(
      ...
      { key: `searchId:${id}`, ttl: 3600 * 3 }
    );
    const meta = await this.#request(
      'GET',
      `/3/tv/${searchId.tv_results[0].id}`,
      { query: { language: language || 'en-US' } },
      { key: `${id}:${language}`, ttl: 3600 * 3 }
    );
